<template>
  <div>
    <div class="h-screen flex flex-col items-center justify-center">
      ...
      <button
        class="bg-red-400 px-4 py-2 mt-2 rounded-lg border-black border-2 mx-2 font-medium"
        @click="joinroom"
      >
        Join Room
      </button>
...
export default {
  ...
  methods:{
    joinroom(event){
      ...
      this.wspath="/ws/"+this.roomname+"/?username="+this.username+'&method=joinroom'
    ...
    validate_data(event){
      if (this.ws_received_data.status=='failed'){
        ...
        this.$store.state.wsconnection.onmessage=null
        this.$router.push({ path: '/lobbyparticipant' }); 
